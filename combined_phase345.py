...
def load_labeled_csv(path: Path, cohort_name: str) -> pd.DataFrame:
    ...
    try:
        # Use compression=None based on previous errors
...
def main():
    ...
    print(f"   Loaded a total of {len(df):,} labeled comments from {len(df['cohort'].unique())} cohorts.")
    ...
    # We need the last_commit_date *from the entire original dataframe*
    # But since df is freed, let's re-calculate it from the adds/rems
    all_comments = pd.concat([adds, rems])
    last_commit_dates = all_comments.groupby('project')['c_committer_date'].max().to_frame('last_commit_date')
    all_comments = None # Free memory
    ...
    fig, (ax_intro, ax_removal) = plt.subplots(1, 2, figsize=(20, 8))
